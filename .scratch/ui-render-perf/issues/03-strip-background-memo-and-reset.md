...
- [ ] 参数为空时视为 reset 保留（`\x1b[0m` 已经保留，行为一致）
- [ ] 加按行字符串的 memo（带上限，如 2000 条），跨帧命中
- [ ] `test/foreign-tool-cards.test.mjs` 补 `\x1b[m` 断言

## 验收

- [ ] `stripBackground("\x1b[m") === "\x1b[m"`
- [ ] `stripBackground("\x1b[48;2;1;2;3mtext\x1b[49m") === "text"` 与另外两条现有断言不变
- [ ] 1000 行输入重复 100 次的总耗时下降一个数量级
