...
import { MODEL_MAX_OUTPUT_TOKENS } from "./limits.generated";
import { GENERATED_MODELS } from "./models.generated";
import { MODEL_PRICING, type ModelPrice } from "./pricing.generated";
import { resolve as resolveThinkingLevelMap } from "./thinking-levels";
import { concurrentMap, fetchJsonWithTimeout, getContextLength } from "./utils";
...
function resolvePrice(id: string): ModelPrice {
  return (
    MODEL_PRICING[id] ?? { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }
  );
}
...
const envBase =
  typeof process !== "undefined" ? process.env?.OLLAMA_API_BASE : undefined;
if (envBase) {
...
export async function fetchModelIds(
  ...
  if (res.status === 429) {
    throw new Error(
      "Ollama Cloud model list fetch rate limited. Try again shortly.",
    );
  }
  if (!res.ok || !res.data) {
    throw new Error(
...
export async function refreshOllamaCloudModels(
  ...
  if (Object.keys(models).length === 0) {
    throw new Error(
      `Failed to fetch model details${failed ? ` (${failed} failed)` : ""}`,
    );
  }
...
export async function refreshOllamaCatalog(
  ...
  // the web fetch tool requires an API key.
  //
  ...
  if (failed === 0) {
    ...
    if (context.stored?.models.length) {
      try {
        const published = await context.publish({
          persist: { ...context.stored, checkedAt: Date.now() },
        });
        if (!published) {
          console.warn(
    ...
    throw new Error(
      `Ollama Cloud catalog refresh incomplete: ${failed} model(s) failed`,
    );
  }
